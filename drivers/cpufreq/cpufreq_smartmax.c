...
#define DEFAULT_AWAKE_IDEAL_FREQ 1497600
 ...
 */
//static unsigned long debug_mask = SMARTMAX_DEBUG_LOAD|SMARTMAX_DEBUG_JUMPS|SMARTMAX_DEBUG_ALG|SMARTMAX_DEBUG_INPUT|SMARTMAX_DEBUG_SUSPEND;
...
static inline u64 get_cpu_idle_time_smartmax(unsigned int cpu, u64 *wall)
{
...
static void inline cpufreq_smartmax_calc_load(int j)
{
	...
	cur_idle_time = get_cpu_idle_time_smartmax(j, &cur_wall_time);
...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_smartmax->prev_cpu_idle = get_cpu_idle_time_smartmax(j,
